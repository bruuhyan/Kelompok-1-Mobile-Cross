...
export default function EmployeeProfileScreen() {
  ...
  const handleAvatarUpload = async (source: 'camera' | 'gallery') => {
    ...
    try {
      ...
      const publicUrl = await storageService.uploadAvatar(user.id, picked.base64);
  ...
  const showAvatarOptions = () => {
    ...
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        {
          ...
          destructiveButtonIndex: user.avatar_url ? options.length - 2 : undefined,
        },
        (buttonIndex) => {
          if (buttonIndex === 0) {
      ...
      Alert.alert('Change Profile Picture', 'Choose an option', [
        ...
        { text: 'Choose from Gallery', onPress: () => handleAvatarUpload('gallery') },
        ...(user.avatar_url
          ? [
              {
                text: 'Remove Photo',
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Card style={styles.profileCard}>
        ...
        {!isUploadingAvatar && (
          <Text style={styles.avatarHint}>Tap profile picture to change photo</Text>
        )}
